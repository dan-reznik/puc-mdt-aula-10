...
for(i in 1:nrow(df_flights))  {
    #Origem
    node1 <- df_airports %>% 
      filter(df_airports$ID == df_flights[i,]$Source)
    #destino
    node2 <- df_airports %>%
      filter(df_airports$ID == df_flights[i,]$Target)
    ...
    map_flights <- addPolylines(map_flights, data = curved_line
                , color = edge_col[edge_ind], weight = edge_ind/30, opacity = 1)
}
# adiciona os pontos por ultimo para sobrepor as linhas
map_flights <- addCircles(map_flights, data = df_airports
                    , lng = ~longitude
                    , lat = ~latitude
                    , color = "red"
                    , popup = pop
                    , weight = 12
                    , opacity = .8
                    ,highlightOptions = highlightOptions(color = "yellow", weight = 24, bringToFront = TRUE))
#mostra o mapa
map_flights
```
