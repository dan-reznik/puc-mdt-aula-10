...
Atribui peso a cada aresta pelo numero de emails

```{r}
E(gr_emails)$weight <- df_emails$emails
```
...
gr_emails %>% 
  ...
  geom_edge_fan2(aes(edge_width=weight)) +
  scale_edge_width_discrete(range(1,100)) +
...
gr_emails %>% 
  ggraph(layout = 'graphopt') + 
  geom_edge_link(aes(edge_width=weight/25),
                arrow = arrow(length = unit(3, 'mm')), 
                 end_cap = circle(3, 'mm')) + 
  geom_node_point(size = 5)
```

Lê info sobre usuários
...
Vamos atribuir os nomes dos usuarios ao grafo

```{r}
V(gr_emails)$name <- df_emails_users$name
V(gr_emails)$sex <- df_emails_users$sex
```


```{r}
gr_emails_attr <- gr_emails %>%
  mutate(name=df_emails_node_names$name,
         color=if_else(df_emails_node_names$sex=="M",'blue','pink'))
```
...
```{r}
links.d3 <- tibble(from=as.numeric(factor(links$from))-1,
